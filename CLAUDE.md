...
- `docs/SPEC.md` — the build contract (currently spec v2.2): full data model, status machine, approval rules, vendor routing, Design Studio integration contract, DID module, two-level access, build order. Authoritative.
...
- `claude-code-sessions.md` — the session-by-session build plan (Sessions 0–8), aligned to spec v2.1 (still accurate; the v2.2 amendment changed §6, not the build order).
...
- Vendor routing resolves per item (item override ?? brand policy) and can split one request into multiple quote packages. **Fulfillment belongs to the PACKAGE, not the request (§6, amended v2.2)**: each package runs its own tail — internal (Signage.com fulfills: quote_ready → accepted → in_production → shipped → installed) or external (email package out; team logs milestones manually) — and the request status is the rollup, sitting at the stage of its least advanced package. A package never blocks its sibling, the same rule §7 gives line items. `completed/installed` is still the ONLY transition that writes installed_signs; it is now the package's, and writes only that package's items.
